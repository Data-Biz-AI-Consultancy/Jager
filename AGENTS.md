...
- **PostgreSQL Database Isolation**: PostgreSQL hosts TWO separate databases:
  ...
  2. `cdp` database: Isolated domain database containing the `cdp` schema (`cdp.leads`, `cdp.persons`, `cdp.companies`, `cdp.engagements`, `cdp.person_account_relationships`).
